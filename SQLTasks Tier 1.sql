...
SELECT name, membercost FROM `Facilities` WHERE membercost > 0 ORDER BY membercost

/* Q2: How many facilities do not charge a fee to members? */
SELECT COUNT(membercost = 0) FROM `Facilities` 
...
in question. */
SELECT name, monthlymaintenance, CASE WHEN monthlymaintenance > 100 THEN 'expensive' ELSE 'cheap' END AS monthlymaintenance_category
FROM `Facilities`
...
SELECT f.name, (SELECT MAX(b.bookid)*(f.guestcost + f.membercost)< 1000 
FROM BOOKINGS AS b, FACILITIES AS f) AS totalrevenue
FROM FACILITIES AS f
ORDER BY totalrevenue

/* Q11: Produce a report of members and who recommended them in alphabetic surname,firstname order */
SELECT surname || ' ' || firstname AS member_name, recommendedby
FROM MEMBERS 
ORDER BY member_name

/* Q12: Find the facilities with their usage by member, but not guests */
SELECT facid, COUNT(memid)
FROM BOOKINGS 
GROUP BY facid
